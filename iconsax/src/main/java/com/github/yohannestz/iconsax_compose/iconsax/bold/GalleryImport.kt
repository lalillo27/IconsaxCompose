package com.github.yohannestz.iconsax_compose.iconsax.bold

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val GalleryImport: ImageVector
    get() {
        val current = _galleryImport
        if (current != null) return current

        return ImageVector.Builder(
            name = "com.github.yohannestz.iconsax_compose.iconsax.IconsaxTheme.GalleryImport",
            defaultWidth = 24.0.dp,
            defaultHeight = 24.0.dp,
            viewportWidth = 24.0f,
            viewportHeight = 24.0f,
        ).apply {
            path(
                fill = SolidColor(Color(0xFF292D32)),
            ) {
                moveTo(x = 20.97f, y = 8.0f)
                horizontalLineToRelative(dx = -2.94f)
                curveTo(x1 = 16.76f, y1 = 8.0f, x2 = 16.0f, y2 = 7.24f, x3 = 16.0f, y3 = 5.97f)
                verticalLineTo(y = 3.03f)
                curveTo(x1 = 16.0f, y1 = 1.76f, x2 = 16.76f, y2 = 1.0f, x3 = 18.03f, y3 = 1.0f)
                horizontalLineToRelative(dx = 2.94f)
                curveTo(x1 = 22.24f, y1 = 1.0f, x2 = 23.0f, y2 = 1.76f, x3 = 23.0f, y3 = 3.03f)
                verticalLineToRelative(dy = 2.94f)
                curveTo(x1 = 23.0f, y1 = 7.24f, x2 = 22.24f, y2 = 8.0f, x3 = 20.97f, y3 = 8.0f)
                moveToRelative(dx = 0.22f, dy = -3.31f)
                arcToRelative(a = 0.6f, b = 0.6f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -0.44f, dy1 = -0.18f)
                arcToRelative(a = 0.6f, b = 0.6f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -0.44f, dy1 = 0.18f)
                lineToRelative(dx = -0.18f, dy = 0.18f)
                verticalLineTo(y = 2.63f)
                curveTo(x1 = 20.13f, y1 = 2.28f, x2 = 19.85f, y2 = 2.0f, x3 = 19.5f, y3 = 2.0f)
                reflectiveCurveToRelative(dx1 = -0.63f, dy1 = 0.28f, dx2 = -0.63f, dy2 = 0.63f)
                verticalLineToRelative(dy = 2.24f)
                lineToRelative(dx = -0.18f, dy = -0.18f)
                arcToRelative(a = 0.63f, b = 0.63f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -0.88f, dy1 = 0.0f)
                arcToRelative(a = 0.63f, b = 0.63f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 0.0f, dy1 = 0.88f)
                lineToRelative(dx = 1.25f, dy = 1.25f)
                arcToRelative(a = 1.0f, b = 1.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 0.19f, dy1 = 0.12f)
                lineToRelative(dx = 0.06f, dy = 0.02f)
                lineToRelative(dx = 0.16f, dy = 0.03f)
                horizontalLineToRelative(dx = 0.06f)
                arcToRelative(a = 1.0f, b = 1.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 0.2f, dy1 = -0.04f)
                horizontalLineToRelative(dx = 0.02f)
                arcToRelative(a = 1.0f, b = 1.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 0.18f, dy1 = -0.12f)
                lineToRelative(dx = 0.02f, dy = -0.01f)
                lineToRelative(dx = 1.25f, dy = -1.25f)
                arcToRelative(a = 0.6f, b = 0.6f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -0.01f, dy1 = -0.88f)
                moveTo(x = 9.0f, y = 10.39f)
                arcToRelative(a = 2.38f, b = 2.38f, theta = 0.0f, isMoreThanHalf = true, isPositiveArc = false, dx1 = 0.0f, dy1 = -4.77f)
                arcToRelative(a = 2.38f, b = 2.38f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 0.0f, dy1 = 4.76f)
            }
            path(
                fill = SolidColor(Color(0xFF292D32)),
            ) {
                moveTo(x = 20.97f, y = 8.0f)
                horizontalLineTo(x = 20.5f)
                verticalLineToRelative(dy = 4.61f)
                lineToRelative(dx = -0.13f, dy = -0.11f)
                arcToRelative(a = 2.2f, b = 2.2f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -2.82f, dy1 = 0.0f)
                lineToRelative(dx = -4.16f, dy = 3.57f)
                arcToRelative(a = 2.2f, b = 2.2f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -2.82f, dy1 = 0.0f)
                lineToRelative(dx = -0.34f, dy = -0.28f)
                arcToRelative(a = 2.2f, b = 2.2f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -2.64f, dy1 = -0.14f)
                lineToRelative(dx = -3.74f, dy = 2.51f)
                arcToRelative(a = 5.0f, b = 5.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -0.35f, dy1 = -1.97f)
                verticalLineTo(y = 7.81f)
                curveToRelative(dx1 = 0.0f, dy1 = -2.82f, dx2 = 1.49f, dy2 = -4.31f, dx3 = 4.31f, dy3 = -4.31f)
                horizontalLineTo(x = 16.0f)
                verticalLineTo(y = 3.03f)
                quadToRelative(dx1 = 0.0f, dy1 = -0.6f, dx2 = 0.23f, dy2 = -1.03f)
                horizontalLineTo(x = 7.81f)
                curveTo(x1 = 4.17f, y1 = 2.0f, x2 = 2.0f, y2 = 4.17f, x3 = 2.0f, y3 = 7.81f)
                verticalLineToRelative(dy = 8.38f)
                arcToRelative(a = 7.0f, b = 7.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 0.56f, dy1 = 2.84f)
                curveTo(x1 = 3.42f, y1 = 20.93f, x2 = 5.26f, y2 = 22.0f, x3 = 7.81f, y3 = 22.0f)
                horizontalLineToRelative(dx = 8.38f)
                curveToRelative(dx1 = 3.64f, dy1 = 0.0f, dx2 = 5.81f, dy2 = -2.17f, dx3 = 5.81f, dy3 = -5.81f)
                verticalLineTo(y = 7.77f)
                arcTo(horizontalEllipseRadius = 2.0f, verticalEllipseRadius = 2.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, x1 = 20.97f, y1 = 8.0f)
            }
        }.build().also { _galleryImport = it }
    }

@Suppress("ObjectPropertyName")
private var _galleryImport: ImageVector? = null
