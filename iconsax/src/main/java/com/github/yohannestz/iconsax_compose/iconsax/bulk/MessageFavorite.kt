package com.github.yohannestz.iconsax_compose.iconsax.bulk

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val MessageFavorite: ImageVector
    get() {
        val current = _messageFavorite
        if (current != null) return current

        return ImageVector.Builder(
            name = "com.github.yohannestz.iconsax_compose.iconsax.IconsaxTheme.MessageFavorite",
            defaultWidth = 24.0.dp,
            defaultHeight = 24.0.dp,
            viewportWidth = 24.0f,
            viewportHeight = 24.0f,
        ).apply {
            path(
                fill = SolidColor(Color(0xFF292D32)),
            ) {
                moveTo(x = 20.97f, y = 1.0f)
                horizontalLineToRelative(dx = -2.94f)
                curveTo(x1 = 16.76f, y1 = 1.0f, x2 = 16.0f, y2 = 1.76f, x3 = 16.0f, y3 = 3.03f)
                verticalLineToRelative(dy = 2.94f)
                curveTo(x1 = 16.0f, y1 = 7.24f, x2 = 16.76f, y2 = 8.0f, x3 = 18.03f, y3 = 8.0f)
                horizontalLineToRelative(dx = 2.94f)
                curveTo(x1 = 22.24f, y1 = 8.0f, x2 = 23.0f, y2 = 7.24f, x3 = 23.0f, y3 = 5.97f)
                verticalLineTo(y = 3.03f)
                curveTo(x1 = 23.0f, y1 = 1.76f, x2 = 22.24f, y2 = 1.0f, x3 = 20.97f, y3 = 1.0f)
                moveToRelative(dx = 0.64f, dy = 3.68f)
                curveTo(x1 = 21.24f, y1 = 5.86f, x2 = 19.95f, y2 = 6.5f, x3 = 19.5f, y3 = 6.5f)
                reflectiveCurveToRelative(dx1 = -1.73f, dy1 = -0.62f, dx2 = -2.11f, dy2 = -1.82f)
                arcToRelative(a = 2.0f, b = 2.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -0.09f, dy1 = -0.57f)
                curveToRelative(dx1 = 0.0f, dy1 = -0.65f, dx2 = 0.33f, dy2 = -1.32f, dx3 = 1.02f, dy3 = -1.54f)
                arcToRelative(a = 1.2f, b = 1.2f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = 1.16f, dy1 = 0.2f)
                arcToRelative(a = 1.3f, b = 1.3f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = 1.18f, dy1 = -0.2f)
                curveToRelative(dx1 = 0.92f, dy1 = 0.29f, dx2 = 1.21f, dy2 = 1.32f, dx3 = 0.95f, dy3 = 2.11f)
            }
            path(
                fill = SolidColor(Color(0xFF292D32)),
                fillAlpha = 0.4f,
                strokeAlpha = 0.4f,
            ) {
                moveTo(x = 20.97f, y = 8.0f)
                horizontalLineToRelative(dx = -2.94f)
                curveTo(x1 = 16.76f, y1 = 8.0f, x2 = 16.0f, y2 = 7.24f, x3 = 16.0f, y3 = 5.97f)
                verticalLineTo(y = 3.03f)
                quadToRelative(dx1 = 0.0f, dy1 = -0.6f, dx2 = 0.22f, dy2 = -1.03f)
                horizontalLineTo(x = 7.0f)
                arcToRelative(a = 5.0f, b = 5.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -5.0f, dy1 = 4.98f)
                verticalLineToRelative(dy = 6.98f)
                arcToRelative(a = 5.0f, b = 5.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 5.0f, dy1 = 4.98f)
                horizontalLineToRelative(dx = 1.5f)
                arcToRelative(a = 1.2f, b = 1.2f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = 0.8f, dy1 = 0.4f)
                lineToRelative(dx = 1.5f, dy = 1.99f)
                curveToRelative(dx1 = 0.66f, dy1 = 0.88f, dx2 = 1.74f, dy2 = 0.88f, dx3 = 2.4f, dy3 = 0.0f)
                lineToRelative(dx = 1.5f, dy = -1.99f)
                arcToRelative(a = 1.0f, b = 1.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = 0.8f, dy1 = -0.4f)
                horizontalLineTo(x = 17.0f)
                arcToRelative(a = 5.0f, b = 5.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 5.0f, dy1 = -4.98f)
                verticalLineTo(y = 7.77f)
                arcTo(horizontalEllipseRadius = 2.0f, verticalEllipseRadius = 2.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, x1 = 20.97f, y1 = 8.0f)
            }
            path(
                fill = SolidColor(Color(0xFF292D32)),
            ) {
                moveTo(x = 12.0f, y = 12.0f)
                curveToRelative(dx1 = -0.56f, dy1 = 0.0f, dx2 = -1.0f, dy2 = -0.45f, dx3 = -1.0f, dy3 = -1.0f)
                reflectiveCurveToRelative(dx1 = 0.45f, dy1 = -1.0f, dx2 = 1.0f, dy2 = -1.0f)
                reflectiveCurveToRelative(dx1 = 1.0f, dy1 = 0.45f, dx2 = 1.0f, dy2 = 1.0f)
                reflectiveCurveToRelative(dx1 = -0.44f, dy1 = 1.0f, dx2 = -1.0f, dy2 = 1.0f)
                moveToRelative(dx = 4.0f, dy = 0.0f)
                curveToRelative(dx1 = -0.56f, dy1 = 0.0f, dx2 = -1.0f, dy2 = -0.45f, dx3 = -1.0f, dy3 = -1.0f)
                reflectiveCurveToRelative(dx1 = 0.45f, dy1 = -1.0f, dx2 = 1.0f, dy2 = -1.0f)
                reflectiveCurveToRelative(dx1 = 1.0f, dy1 = 0.45f, dx2 = 1.0f, dy2 = 1.0f)
                reflectiveCurveToRelative(dx1 = -0.44f, dy1 = 1.0f, dx2 = -1.0f, dy2 = 1.0f)
                moveToRelative(dx = -8.0f, dy = 0.0f)
                curveToRelative(dx1 = -0.56f, dy1 = 0.0f, dx2 = -1.0f, dy2 = -0.45f, dx3 = -1.0f, dy3 = -1.0f)
                reflectiveCurveToRelative(dx1 = 0.45f, dy1 = -1.0f, dx2 = 1.0f, dy2 = -1.0f)
                reflectiveCurveToRelative(dx1 = 1.0f, dy1 = 0.45f, dx2 = 1.0f, dy2 = 1.0f)
                reflectiveCurveToRelative(dx1 = -0.44f, dy1 = 1.0f, dx2 = -1.0f, dy2 = 1.0f)
            }
        }.build().also { _messageFavorite = it }
    }

@Suppress("ObjectPropertyName")
private var _messageFavorite: ImageVector? = null
