package com.github.yohannestz.iconsax_compose.iconsax.bold

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val BackwardFifteenSeconds: ImageVector
    get() {
        val current = _backwardFifteenSeconds
        if (current != null) return current

        return ImageVector.Builder(
            name = "com.github.yohannestz.iconsax_compose.iconsax.IconsaxTheme.BackwardFifteenSeconds",
            defaultWidth = 24.0.dp,
            defaultHeight = 24.0.dp,
            viewportWidth = 24.0f,
            viewportHeight = 24.0f,
        ).apply {
            path(
                fill = SolidColor(Color(0xFF292D32)),
            ) {
                moveTo(x = 14.43f, y = 16.92f)
                horizontalLineToRelative(dx = -2.29f)
                curveToRelative(dx1 = -0.4f, dy1 = 0.0f, dx2 = -0.75f, dy2 = -0.34f, dx3 = -0.75f, dy3 = -0.75f)
                reflectiveCurveToRelative(dx1 = 0.34f, dy1 = -0.75f, dx2 = 0.75f, dy2 = -0.75f)
                horizontalLineToRelative(dx = 2.3f)
                arcToRelative(a = 0.78f, b = 0.78f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 0.0f, dy1 = -1.56f)
                horizontalLineToRelative(dx = -2.3f)
                arcToRelative(a = 0.8f, b = 0.8f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -0.6f, dy1 = -0.31f)
                arcToRelative(a = 0.8f, b = 0.8f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -0.1f, dy1 = -0.68f)
                lineToRelative(dx = 0.75f, dy = -2.29f)
                arcToRelative(a = 0.74f, b = 0.74f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = 0.71f, dy1 = -0.51f)
                horizontalLineToRelative(dx = 3.06f)
                curveToRelative(dx1 = 0.41f, dy1 = 0.0f, dx2 = 0.75f, dy2 = 0.34f, dx3 = 0.75f, dy3 = 0.75f)
                reflectiveCurveToRelative(dx1 = -0.34f, dy1 = 0.75f, dx2 = -0.75f, dy2 = 0.75f)
                horizontalLineToRelative(dx = -2.52f)
                lineToRelative(dx = -0.26f, dy = 0.79f)
                horizontalLineToRelative(dx = 1.25f)
                arcToRelative(a = 2.28f, b = 2.28f, theta = 0.0f, isMoreThanHalf = true, isPositiveArc = true, dx1 = 0.0f, dy1 = 4.56f)
                moveToRelative(dx = -4.89f, dy = 0.0f)
                arcToRelative(a = 0.76f, b = 0.76f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -0.75f, dy1 = -0.75f)
                verticalLineToRelative(dy = -3.39f)
                lineTo(x = 8.6f, y = 13.0f)
                arcToRelative(a = 0.76f, b = 0.76f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -1.06f, dy1 = 0.06f)
                arcTo(horizontalEllipseRadius = 0.76f, verticalEllipseRadius = 0.76f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, x1 = 7.49f, y1 = 12.0f)
                lineToRelative(dx = 1.5f, dy = -1.67f)
                arcToRelative(a = 0.75f, b = 0.75f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = 1.31f, dy1 = 0.5f)
                verticalLineToRelative(dy = 5.35f)
                arcToRelative(a = 0.76f, b = 0.76f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -0.76f, dy1 = 0.74f)
            }
            path(
                fill = SolidColor(Color(0xFF292D32)),
            ) {
                moveTo(x = 12.0f, y = 3.48f)
                lineToRelative(dx = -0.24f, dy = 0.01f)
                lineToRelative(dx = 0.82f, dy = -1.02f)
                arcToRelative(a = 0.74f, b = 0.74f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -0.12f, dy1 = -1.05f)
                arcToRelative(a = 0.75f, b = 0.75f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -1.05f, dy1 = 0.12f)
                lineTo(x = 9.44f, y = 4.0f)
                lineTo(x = 9.42f, y = 4.04f)
                lineTo(x = 9.35f, y = 4.17f)
                lineTo(x = 9.3f, y = 4.3f)
                verticalLineToRelative(dy = 0.34f)
                quadToRelative(dx1 = 0.0f, dy1 = 0.04f, dx2 = 0.03f, dy2 = 0.08f)
                lineTo(x = 9.4f, y = 4.86f)
                lineToRelative(dx = 0.1f, dy = 0.11f)
                quadToRelative(dx1 = 0.02f, dy1 = 0.05f, dx2 = 0.06f, dy2 = 0.08f)
                lineTo(x = 9.6f, y = 5.08f)
                lineToRelative(dx = 0.08f, dy = 0.04f)
                lineToRelative(dx = 0.16f, dy = 0.06f)
                quadTo(x1 = 9.9f, y1 = 5.2f, x2 = 9.94f, y2 = 5.2f)
                lineToRelative(dx = 0.08f, dy = 0.01f)
                lineToRelative(dx = 0.07f, dy = -0.02f)
                horizontalLineToRelative(dx = 0.1f)
                arcToRelative(a = 8.15f, b = 8.15f, theta = 0.0f, isMoreThanHalf = true, isPositiveArc = true, dx1 = -6.33f, dy1 = 7.92f)
                arcTo(horizontalEllipseRadius = 8.0f, verticalEllipseRadius = 8.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, x1 = 5.5f, y1 = 8.25f)
                arcToRelative(a = 0.75f, b = 0.75f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -1.2f, dy1 = -0.9f)
                arcToRelative(a = 9.5f, b = 9.5f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -1.95f, dy1 = 5.76f)
                arcToRelative(a = 9.65f, b = 9.65f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 19.28f, dy1 = 0.0f)
                arcTo(horizontalEllipseRadius = 9.63f, verticalEllipseRadius = 9.63f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, x1 = 12.0f, y1 = 3.48f)
            }
        }.build().also { _backwardFifteenSeconds = it }
    }

@Suppress("ObjectPropertyName")
private var _backwardFifteenSeconds: ImageVector? = null
