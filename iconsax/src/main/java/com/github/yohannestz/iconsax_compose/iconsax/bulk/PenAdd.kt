package com.github.yohannestz.iconsax_compose.iconsax.bulk

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val PenAdd: ImageVector
    get() {
        val current = _penAdd
        if (current != null) return current

        return ImageVector.Builder(
            name = "com.github.yohannestz.iconsax_compose.iconsax.IconsaxTheme.PenAdd",
            defaultWidth = 24.0.dp,
            defaultHeight = 24.0.dp,
            viewportWidth = 24.0f,
            viewportHeight = 24.0f,
        ).apply {
            path(
                fill = SolidColor(Color(0xFF292D32)),
            ) {
                moveTo(x = 21.71f, y = 11.35f)
                lineToRelative(dx = -3.0f, dy = 1.9f)
                lineToRelative(dx = -6.09f, dy = -6.08f)
                lineToRelative(dx = 1.9f, dy = -3.01f)
                curveToRelative(dx1 = 0.87f, dy1 = -1.39f, dx2 = 2.64f, dy2 = -1.45f, dx3 = 3.93f, dy3 = -0.16f)
                lineToRelative(dx = 3.41f, dy = 3.41f)
                curveToRelative(dx1 = 1.22f, dy1 = 1.23f, dx2 = 1.14f, dy2 = 3.12f, dx3 = -0.15f, dy3 = 3.94f)
            }
            path(
                fill = SolidColor(Color(0xFF292D32)),
                fillAlpha = 0.4f,
                strokeAlpha = 0.4f,
            ) {
                moveTo(x = 16.0f, y = 21.29f)
                lineToRelative(dx = -9.36f, dy = 1.1f)
                arcToRelative(a = 3.0f, b = 3.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -1.72f, dy1 = -0.26f)
                arcToRelative(a = 2.6f, b = 2.6f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -1.19f, dy1 = -1.19f)
                arcToRelative(a = 3.0f, b = 3.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -0.26f, dy1 = -1.7f)
                lineToRelative(dx = 1.04f, dy = -8.71f)
                arcTo(horizontalEllipseRadius = 4.27f, verticalEllipseRadius = 4.27f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, x1 = 6.5f, y1 = 11.0f)
                arcToRelative(a = 4.4f, b = 4.4f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 2.96f, dy1 = -1.12f)
                arcToRelative(a = 4.5f, b = 4.5f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 1.37f, dy1 = -2.18f)
                arcToRelative(a = 4.0f, b = 4.0f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 0.13f, dy1 = -0.63f)
                lineToRelative(dx = 1.66f, dy = 0.1f)
                lineToRelative(dx = 6.08f, dy = 6.09f)
                lineToRelative(dx = 0.26f, dy = 4.42f)
                curveToRelative(dx1 = 0.25f, dy1 = 2.48f, dx2 = -0.61f, dy2 = 3.34f, dx3 = -2.96f, dy3 = 3.61f)
            }
            path(
                fill = SolidColor(Color(0xFF292D32)),
            ) {
                moveTo(x = 8.78f, y = 18.28f)
                lineToRelative(dx = -3.85f, dy = 3.85f)
                arcToRelative(a = 2.5f, b = 2.5f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = -1.19f, dy1 = -1.19f)
                lineToRelative(dx = 3.85f, dy = -3.85f)
                arcToRelative(a = 0.84f, b = 0.84f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = 1.19f, dy1 = 0.0f)
                arcToRelative(a = 0.84f, b = 0.84f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = true, dx1 = 0.0f, dy1 = 1.2f)
                moveTo(x = 6.5f, y = 2.0f)
                arcToRelative(a = 4.5f, b = 4.5f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = -2.81f, dy1 = 0.99f)
                arcTo(horizontalEllipseRadius = 4.5f, verticalEllipseRadius = 4.5f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, x1 = 2.0f, y1 = 6.5f)
                arcToRelative(a = 4.5f, b = 4.5f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 7.46f, dy1 = 3.38f)
                arcToRelative(a = 4.5f, b = 4.5f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, dx1 = 1.37f, dy1 = -2.18f)
                arcTo(horizontalEllipseRadius = 4.5f, verticalEllipseRadius = 4.5f, theta = 0.0f, isMoreThanHalf = false, isPositiveArc = false, x1 = 6.5f, y1 = 2.0f)
                moveToRelative(dx = 1.66f, dy = 5.25f)
                horizontalLineTo(x = 7.27f)
                verticalLineToRelative(dy = 0.93f)
                curveToRelative(dx1 = 0.0f, dy1 = 0.41f, dx2 = -0.34f, dy2 = 0.75f, dx3 = -0.75f, dy3 = 0.75f)
                reflectiveCurveTo(x1 = 5.77f, y1 = 8.59f, x2 = 5.77f, y2 = 8.18f)
                verticalLineTo(y = 7.25f)
                horizontalLineTo(x = 4.8f)
                curveToRelative(dx1 = -0.41f, dy1 = 0.0f, dx2 = -0.75f, dy2 = -0.34f, dx3 = -0.75f, dy3 = -0.75f)
                reflectiveCurveTo(x1 = 4.39f, y1 = 5.75f, x2 = 4.8f, y2 = 5.75f)
                horizontalLineToRelative(dx = 0.97f)
                verticalLineTo(y = 4.82f)
                curveToRelative(dx1 = 0.0f, dy1 = -0.41f, dx2 = 0.34f, dy2 = -0.75f, dx3 = 0.75f, dy3 = -0.75f)
                reflectiveCurveToRelative(dx1 = 0.75f, dy1 = 0.34f, dx2 = 0.75f, dy2 = 0.75f)
                verticalLineToRelative(dy = 0.93f)
                horizontalLineToRelative(dx = 0.89f)
                arcToRelative(a = 0.75f, b = 0.75f, theta = 0.0f, isMoreThanHalf = true, isPositiveArc = true, dx1 = 0.0f, dy1 = 1.5f)
            }
        }.build().also { _penAdd = it }
    }

@Suppress("ObjectPropertyName")
private var _penAdd: ImageVector? = null
